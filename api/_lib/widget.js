...
export function resolveQuickActions(row) {
  ...
  return stored
    ...
    .slice(0, MAX_ACTIONS)
    .map((a) => ({ key: String(a.key || "").slice(0, 24), label: String(a.label).slice(0, MAX_LABEL), prompt: String(a.prompt).slice(0, MAX_PROMPT) }));
}
...
export function sanitizeQuickActions(raw) {
  ...
  for (const a of raw.slice(0, MAX_ACTIONS)) {
    const label = String((a && a.label) || "").trim().slice(0, MAX_LABEL);
    const prompt = String((a && a.prompt) || "").trim().slice(0, MAX_PROMPT);
    if (!label || !prompt) continue;
    // Keys identify a button across saves; de-duplicate so one can't shadow another.
    let key = String((a && a.key) || label).toLowerCase().replace(/[^a-z0-9]+/g, "-").slice(0, 24) || "action";
